...
export async function saveOnboarding(data: {
  ...
  try {
    ...
    // Check if user has an existing active subscription or any subscription
    const { data: profile } = await admin.from('users').select('subscription_end').eq('id', data.userId).single()

    // Add 1-day free trial on first onboarding if they don't have one
    let subEnd = profile?.subscription_end
    if (!subEnd) {
      const trialDate = new Date()
      trialDate.setDate(trialDate.getDate() + 1)
      subEnd = trialDate.toISOString()
    }
    ...
    console.error('Onboarding catch error:', e)
